        ...
        for line in in_file.readlines():
            if len(line) > len(INS_PREAMBLE):
                ...
                if (
                    ...
                    line = preamble + data + ("0" * (num_shares - 1) * len(data)) + "\n"
                    if w_nbytes == 0:
                    ...
                    for i in range(0, len(data_bytes), w_nbytes):
                        ...
                        shares = [
                            secrets.token_bytes(w_nbytes) for _ in range(num_shares - 1)
                        ]
...
def main(args):
    if args.design:
        try:
            ...
            try:
                ...
                print(
                    "tomli package needs to be installed. Try running: 'python3 -m pip install -U tomli'"
                )
                sys.exit(1)
    ...
    if args.folder:
        ...
        assert (
            args.folder.exists() and args.folder.is_dir()
...
def run(args: List[str]) -> None:
    parser = argparse.ArgumentParser(description="Generates shared testvectors and RDI")

    parser.add_argument(
        "--rdi-file", default=None, type=Path, help="path to generated rdi.txt"
    )
    parser.add_argument(
        "--pdi-file",
        default=None,
    ...
    parser.add_argument(
        "--rdi-width",
        default=None,
        required="--rdi-file" in sys.argv,
        type=int,
        help="width of RDI data port in bits (RW)",
    )
    parser.add_argument(
        "--pdi-width", default=None, type=int, help="width of PDI data port in bits (W)"
    )
    parser.add_argument(
        "--sdi-width",
        default=None,
    ...
    parser.add_argument(
        "--pdi-shares",
        default=None,
        required="--pdi-file" in sys.argv,
        type=int,
        help="number of PDI shares",
    )
    parser.add_argument(
        "--sdi-shares", default=None, type=int, help="number of SDI shares"
    )
    parser.add_argument(
        "--rdi-words", default=200000, type=int, help="number of RDI words"
    )
    parser.add_argument(
        "--design",
        default=None,
